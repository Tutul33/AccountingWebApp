...
@Component({
  ...
  providers: [ContraVoucherModelService],
  standalone:true,
  imports:[SharedModule]
})
export class ContraVoucherListComponent extends BaseComponent implements OnInit {
  ...
  getContraVourcherList() {
    try {
      ...
      this.spData = new QueryData({
        ...
        queryEvent: 'contraVoucherListComponent',
        pageNo: 1,
    });
  ...
  populateGrid(isRefresh: boolean) {
    try {
      ...
      this.commonSvc.getVoucherList(this.spData, this.modelSvc.searchParam.companyID, this.modelSvc.searchParam.fromDate, this.modelSvc.searchParam.toDate,  this.modelSvc.searchParam.voucherTypeCD, null, null, this.userId).subscribe({
        next: (res: any) => {
          if (isRefresh == true) this.bindDataDDLPropertiesData(res);debugger
          this.modelSvc.contraVourcherList = this.modelSvc.prepareVoucherList(res[res.length - 1] || []);
  ...
  bindDataDDLPropertiesData(data: any) {
    try {
      ...
      this.modelSvc.gridDebitList = data[1].filter(f => f.DebitVal != 0);
      this.modelSvc.gridCreidList = data[2].filter(f => f.CreditVal != 0);
      this.modelSvc.gridCreatedByList = data[3];
      this.modelSvc.gridModifiedByList = data[4];
      this.modelSvc.gridDraftStatusLlist = data[5];
  ...
  initGridOption() {
    try {
      const gridObj = {
        dataSource: "modelSvc.contraVourcherList",
  ...
  {
    try {
      event.exportOption.title =this.fieldTitle['contravoucherlist'] + "_" + formatDate(this.modelSvc.searchParam.fromDate, "yyyy", "en") + "_" + this.modelSvc.searchParam.companyName;
  ...
  prepareDataForEdit(entity: any, companyID: any) {
    try {
      ...
      let navigateUrl = '/ACC-PAGE/contra-voucher-entry';
