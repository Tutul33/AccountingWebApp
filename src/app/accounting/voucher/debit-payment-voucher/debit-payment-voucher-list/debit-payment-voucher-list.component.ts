...
@Component({
  selector: 'app-debit-payment-voucher-list',
  templateUrl: './debit-payment-voucher-list.component.html',
  providers: [DebitPaymentVoucherModelService],
  standalone: true,
  imports: [SharedModule]
})
export class DebitPaymentVoucherListComponent extends BaseComponent implements OnInit {
    ...
    try {
      forkJoin([
        ...
        next: (res: any) => {
          ...
          this.getDebitPaymentVourcherList();
  ...
  getDebitPaymentVourcherList() {
    try {
      ...
      this.spData = new QueryData({
        ...
        queryEvent: 'debitPaymentVoucherListComponent',
        pageNo: 1,
      });
  ...
  populateGrid(isRefresh: boolean) {
    try {
      ...
      this.commonSvc.getVoucherList(this.spData, this.modelSvc.searchParam.companyID, this.modelSvc.searchParam.fromDate, this.modelSvc.searchParam.toDate, this.modelSvc.searchParam.voucherTypeCD, null, null, this.userId).subscribe({
        next: (res: any) => {
          if (isRefresh == true) this.bindDataDDLPropertiesData(res);
          this.modelSvc.debitPaymentVourcherList = this.modelSvc.prepareVoucherList(res[res.length - 1] || []);
  ...
  bindDataDDLPropertiesData(data: any) {
    try {
      ...
      this.modelSvc.gridInvBillList = data[7];
      this.modelSvc.gridChequeNoList = data[8];
      this.modelSvc.gridChequeIssueDateList = data[9];
      this.modelSvc.gridChequeClearedOnDateList = data[10];
      this.modelSvc.gridDraftStatusLlist = data[11];
  ...
  initGridOption() {
    try {
      const gridObj = {
        dataSource: "modelSvc.debitPaymentVourcherList",
  ...
  gridColumns(): ColumnType[] {
    return [
      ...
      { field: "chequeNo", header: this.fieldTitle['chequeno'], isDDFilter: true, dataList: this.modelSvc.gridChequeNoList, labelField: 'ChequeNo' },
      { field: "chequeDate", header: this.fieldTitle['chequeissuedate'], isDDFilter: true, dataList: this.modelSvc.gridChequeIssueDateList, labelField: 'ChequeDate' },
      { field: "clearedOnDate", header: this.fieldTitle['clearedondate'], isDDFilter: true, dataList: this.modelSvc.gridChequeClearedOnDateList, labelField: 'ClearedOnDate' },
      { field: "description", header: this.fieldTitle['description'] },
      { field: "debitVal", header: this.fieldTitle['debitamount(bdt)'], isDDFilter: true, dataList: this.modelSvc.gridDebitList, labelField: 'DebitVal' },
      { field: "creditVal", header: this.fieldTitle['creditamount(bdt)'], isDDFilter: true, dataList: this.modelSvc.gridCreidList, labelField: 'CreditVal' },
      { field: "invoiceBillRefNo", header: this.fieldTitle['invoicebillrefno'], isDDFilter:true, dataList: this.modelSvc.gridInvBillList, labelField: 'InvoiceBillRefNo' },
  ...
  getDataToExport(event: any) {
    try {
      event.exportOption.title = this.fieldTitle['debitpaymentvoucherlist'] + "_" + formatDate(this.modelSvc.searchParam.fromDate, "yyyy", "en") + "_" + this.modelSvc.searchParam.companyName;

      let spData = GlobalMethods.jsonDeepCopy(this.spData);
      spData.pageNo = 0;

      return this.commonSvc.getVoucherList(spData, this.modelSvc.searchParam.companyID, this.modelSvc.searchParam.fromDate, this.modelSvc.searchParam.toDate, this.modelSvc.searchParam.voucherTypeCD, null, null, this.userId).pipe(
        map((response: any) => {
    ...
    try {
      this.utilitySvc
        ...
        .subscribe((isConfirm: boolean) => {
          if (isConfirm) {
            this.commonSvc.delete(entity.id,entity.voucherItemID).subscribe({
              next: (res: any) => {
  ...
  printCheque(entity: any) {
    try {
      const modalConfig = new ModalConfig();
      const obj = { entity: entity };
      modalConfig.data = obj;
      modalConfig.header = this.fieldTitle['printcheque'];
      this.ref = this.dialogService.open(ChequePrintModalComponent, modalConfig);
      this.ref.onClose.subscribe((data: any) => {
        if (data != undefined) {
          this.coreAccountingSvc.printCheque(entity.voucherItemID, data); //for print cheque
          this.coreAccountingSvc.saveSoftCheque(entity.voucherItemID, data); // for save data and soft copy
        }
  ...
  sendEmail(entity: any) {
    try {
      this.coreAccountingSvc.getVoucherReportFileName(entity.voucherTypeCd, entity.id).subscribe({
        next: (res: any) => {
          this.showEmailModal(entity, res);
        },
        error: (e: any) => {
          this.showErrorMsg(e);
        }
  ...
  showEmailModal(entity: any, reportName: string) {
    try {
      let obj = {
        attachmentName: [],
      ...
      const ref = this.dialogService.open(EmailSendComponent, {
        data: obj,
  ...
  saveNotification(data: any) {
    ...
    //Send the above object to the service
    list.push(obj);

    if (list.length) {
      this.coreAccountingSvc.saveNotification(list).subscribe({
        next: (res: any) => {
