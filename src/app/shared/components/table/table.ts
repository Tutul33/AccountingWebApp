import {
    ...
    Input, Output, EventEmitter, ElementRef, ContentChildren, TemplateRef, QueryList, ViewChild, NgZone, ChangeDetectorRef, OnChanges, SimpleChanges, ChangeDetectionStrategy, Query, ViewEncapsulation, Renderer2, TrackByFunction
} from '@angular/core';
import { CommonModule, formatDate } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { PrimeNG } from 'primeng/config';
import { PrimeTemplate, SharedModule, FilterMatchMode, FilterOperator, SelectItem, TranslationKeys, FilterService, OverlayService } from 'primeng/api';
...
@Component({
    ...
    template: `
        <div #container [ngStyle]="style" [class]="styleClass"
            ...
            <div #wrapper class="p-datatable-wrapper" [ngStyle]="{height: scrollHeight}">
                <table #table *ngIf="!virtualScroll" role="table" [ngClass]="{'p-datatable-table':true,'p-frozen-scrollable':hasFrozenColumn}" [ngStyle]="tableStyle"><!--[ngClass]="tableStyleClass" class="p-datatable-table p-frozen-scrollable" -->
                ...
                <cdk-virtual-scroll-viewport *ngIf="virtualScroll" [itemSize]="virtualRowHeight" tabindex="0" [style.height]="scrollHeight !== 'flex' ? scrollHeight : undefined" [minBufferPx]="minBufferPx" [maxBufferPx]="maxBufferPx" (scrolledIndexChange)="onScrollIndexChange($event)" class="p-datatable-virtual-scrollable-body">
                    <table #table role="table" [ngClass]="{'p-datatable-table':true,'p-frozen-scrollable':hasFrozenColumn}" [ngStyle]="tableStyle">
                        <ng-container *ngTemplateOutlet="colGroupTemplate; context {$implicit: columns}"></ng-container>
            ...
            <div *ngIf="isClearTemplate" class="p-datatable-footer">
                <ng-container *ngTemplateOutlet="isClearTemplate"></ng-container>
            </div>
    ...
    standalone:true,
    imports:[CommonModule,FormsModule,PaginatorModule,ScrollingModule,TableBody]
})
export class Table implements OnInit, AfterViewInit, AfterContentInit, BlockableUI, OnChanges {
    ...
    // Modified by mozahedul Islam
    @Input() filters: { [s: string]: FilterMetadata | any } = {};
    ...
    isClearTemplate: TemplateRef<any>;
        ...
        this.templates.forEach((item) => {
            switch (item.getType()) {
                ...
                case 'paginatordropdownitem':
                    ...
                    break;
                case 'isClear':
                    this.isClearTemplate = item.template;
                    break;
            }
        });
    }
        ...
        if(data.length>0){
            if(this.paginator && !this.lazy){
                ...
                if(data.length>this.first){
                    returnData=data.slice(this.first, this.first + this.rows);
                }else{                    
                    //To stop hiding filtered data, on click column filter 
                    //If total data less than first index then start from 0 index
                    this.first=0;
    ...
    onPageChange(event) {
        ...
        if (this.lazy) {
            const isFromPaging = true;
            this.onLazyLoad.emit(this.createLazyLoadMetadata(isFromPaging));
        }
    ...
    _filter() {
        ...
        if (this.lazy) {
            ...
            if (!this.hasFilter()) {
                ...
                if (this.value.length > 0) {
                    for (let prop in this.value[0]) {
                        if (Array.isArray(this.value[0][prop])) {
                            arrayProperty = GlobalMethods.jsonDeepCopy(prop);;
                            break;
                        }
                    }
                }
                let arrayPropertyInFilter = '';
                for (let prop in this.filters) {
                    if (Array.isArray(this.filters[prop])) {
                        arrayPropertyInFilter = GlobalMethods.jsonDeepCopy(prop);
                        break;
                    }
                }
                ...
                for (let i = 0; i < this.value.length; i++) {
                    ...
                    for (let prop in this.filters) {
                        ...
                        if (this.filters.hasOwnProperty(prop) && prop !== 'global') {
                            ...
                            if (arrayPropertyInFilter != '') {
                                for (let meta of filterMeta) {
                                    localMatch = this.executeLocalFilter(prop, entity, meta);
                                ...
                                localMatch = this.executeLocalFilter(prop, entity, filterMeta);
                            }
                    ...
                    if (this.filters['global'] && !this.globalMatch && globalFilterFieldsArray) {
                        for (let j = 0; j < globalFilterFieldsArray.length; j++) {
                            let globalFilterField = globalFilterFieldsArray[j].field || globalFilterFieldsArray[j];
                            this.globalMatch = this.filterService.filters[(<FilterMetadata>this.filters['global']).matchMode](ObjectUtils.resolveFieldData(entity, globalFilterField), (<FilterMetadata>this.filters['global']).value.trim(), this.filterLocale);
                        ...
                        if (!this.globalMatch && arrayProperty != '') {
                            if (entity[arrayProperty].length > 0 && entity[arrayProperty][0] != '') {
                                this._deepSearch(entity[arrayProperty], (<FilterMetadata>this.filters['global']).value.trim().toLowerCase(), globalFilterFieldsArray);
                            }
                        }
                    }
                    let matches: boolean;
                    if (this.filters['global']) {
                        matches = localFiltered ? (localFiltered && localMatch && this.globalMatch) : this.globalMatch;
                    }
                    else {
        ...
        this.onFilter.emit({
            ...
            isFilterdValueExist: this.filteredValue ? this.filteredValue.length ? true : false : false,
            originalfilteredValue: this.filteredValue ? this.filteredValue.length ? this.filteredValue : [] : []
        });
    ...
    _resetValue(dataList: any) {
        dataList.forEach(element => {
            element.isHideRow = false;
            for (let prop in element) {
                let filterMeta = element[prop];
                if (Array.isArray(filterMeta)) {
                    if (filterMeta.length > 0 && filterMeta[0] != '')
                        this._resetValue(filterMeta);
                }
            }
        });
    }

    _deepSearch(dataList: any, searchValue: any, globalFilterFieldsArray: any) {
        let arrayProperty = '';
        for (let prop in dataList[0]) {
            let filterMeta = dataList[0][prop];
            if (Array.isArray(filterMeta)) {
                arrayProperty = GlobalMethods.jsonDeepCopy(prop);;
                break;
            }
        ...
        dataList.forEach(element => {
            ...
            for (let prop in element) {
                ...
                if (filterMeta != null && filterMeta != undefined) {
                    if (globalFilterFieldsArray.length > 0) {
                        let fieldValues = globalFilterFieldsArray.toString().toLowerCase();
                        if (fieldValues.includes(prop.toLowerCase())) {
                            isFilter = true;
                        }
                    }
                    else {
                    ...
                    if (isFilter) {
                        let filterMetaValue = filterMeta.toString().toLowerCase();
                        filterMetaValue = filterMetaValue == "true" ? filterMetaValue = "yes" : filterMetaValue == "false" ? filterMetaValue = "no" : filterMetaValue;
                        if (filterMetaValue.includes(searchValue)) {
                            isMatch = true;
                            break;
                        }
                    }
            ...
            if (!isMatch) {
                if (arrayProperty != '') {
                    if (element[arrayProperty].length > 0) {
                        this._deepSearch(element[arrayProperty], searchValue, globalFilterFieldsArray);
                        let isChildMatch = element[arrayProperty].find(f => f.isHideRow == false);
                        if (isChildMatch) {
                            isMatch = true;
                        }
                    } else {
                        element.isHideRow = true;
                    }
                }
                else {
                    element.isHideRow = true;
            ...
            if (isMatch) {
                element.isHideRow = false;
                if (!this.globalMatch) this.globalMatch = true;
            }
            else {
                element.isHideRow = true;
            }
    ...
    // Added by mozahed for filtering data against array
    filterForReport(reportData: any[]) {
        if (!reportData) {
            return [];
        }
        ...
        for (let i = 0; i < reportData.length; i++) {
            ...
            for (let prop in this.filters) {
                if (this.filters.hasOwnProperty(prop) && prop !== 'global') {
                    ...
                    if (Array.isArray(filterMeta)) {
                        for (let meta of filterMeta) {
                            localMatch = this.executeLocalFilter(filterField, reportData[i], meta);
                        ...
                        localMatch = this.executeLocalFilter(filterField, reportData[i], filterMeta);
                    }
            ...
            if (this.filters['global'] && !globalMatch && globalFilterFieldsArray) {
                for (let j = 0; j < globalFilterFieldsArray.length; j++) {
                    let globalFilterField = globalFilterFieldsArray[j].field || globalFilterFieldsArray[j];
                    globalMatch = this.filterService.filters[(<FilterMetadata>this.filters['global']).matchMode](ObjectUtils.resolveFieldData(reportData[i], globalFilterField), (<FilterMetadata>this.filters['global']).value, this.filterLocale);
            ...
            if (matches) {
                filteredArray.push(reportData[i]);
            }
        }
    ...
    executeLocalFilter(field: string, rowData: any, filterMeta: FilterMetadata): boolean {
        ...
        let value: any = JSON.parse(JSON.stringify(filterMeta.value));

        if (filterMeta.value instanceof Date) {
            let convertedValue = formatDate(filterMeta.value, 'dd-MMM-yy', "en");
            value = new Date(convertedValue);
            let fieldVal = formatDate(new Date(dataFieldValue), 'dd-MMM-yy', "en");
            dataFieldValue = new Date(fieldVal);
        }

        if (filterMeta.value && filterMeta.matchMode == 'in') {
            value = value.split(',').map(function (item: any) {
                return item.trim();
            });
        }

        if (typeof dataFieldValue == 'number') dataFieldValue = dataFieldValue.toString();
    ...
    createLazyLoadMetadata(isFromPage?: any): any {
        return {
            first: this.first,
            rows: this.rows,
    ...
    public exportCSV(options?: any, title?: string) {
        let data;
        let csv = '';
        ...
        if (options && options.selectionOnly) {
            ...
            if (this.filteredValue) {
                data = JSON.parse(JSON.stringify(this.filteredValue));
            }
            else if (this.value) {
            ...
            if (this.frozenValue) {
                frozenValue = JSON.parse(JSON.stringify(this.frozenValue));
            }
            if (frozenValue) {
        ...
        data.forEach((record, i) => {
            ...
            for (let i = 0; i < columns.length; i++) {
                ...
                if (column.exportable !== false && column.field) {
                    ...
                    // Wrap cell data with double quotes and escape quotes inside
        ...
        // Add BOM (Byte Order Mark) and encode CSV data to UTF-8
        let csvData = new Uint8Array([0xef, 0xbb, 0xbf]); // BOM for UTF-8
        csvData = Uint8Array.from([...csvData, ...new TextEncoder().encode(csv)]);
    
        // Create Blob with UTF-8 encoded CSV data
        let utf8Blob = new Blob([csvData], { type: 'text/csv;charset=utf-8;' });
    
        // Save or open the Blob based on browser support
        if ((<any>window.navigator).msSaveOrOpenBlob) {
            (<any>window.navigator).msSaveOrOpenBlob(utf8Blob, title + '.csv');
        } else {
            let link = document.createElement('a');
            link.style.display = 'none';
            link.href = window.URL.createObjectURL(utf8Blob);
...
export class RowToggler {
    ...
    @Input('pRowTogglerCustom') customdata?: any;
    ...
    onClick(event: Event) {
        if (this.isEnabled()) {
            this.dt.toggleRow(this.customdata ? this.customdata : this.data, event);
            event.preventDefault();
        }
    }
    ...
    template: `
        ...
        <ng-template #builtInElement>
            <ng-container [ngSwitch]="type">
                <input *ngSwitchCase="'text'" type="text" pInputText [value]="filterConstraint?.value" (input)="onModelChange($any($event.target).value)"
                ...
                <p-datepicker appendTo="body" *ngSwitchCase="'date'" dateFormat="dd-M-y" (ngModelChange)="onModelChange($event)" [ngModel]="filterConstraint?.value"></p-datepicker>
            </ng-container>
        </ng-template>
    `,
    encapsulation: ViewEncapsulation.None,
...
export class ColumnFilterFormElement implements OnInit {
    ...
    ngOnInit() {
        this.filterCallback = (value: any) => {
            ...
            // this.dt._filter(); // Edit by mozahedul
        };
    }
    ...
    onTextInputEnterKeyDown(event: KeyboardEvent | any) {
        this.dt._filter();
        event.preventDefault();
    }
...
@Component({
            ...
            <div *ngIf="showMenu && overlayVisible" [ngClass]="{'p-column-filter-overlay p-component p-fluid': true, 'p-column-filter-overlay-menu': display === 'menu'}" (click)="onContentClick()"
                ...
                <ng-template #menu>
                    <div class="p-column-filter-operator" *ngIf="isShowOperator">
                        <p-select appendTo="body" appendTo="body" appendTo="body" [options]="operatorOptions" [ngModel]="operator" (ngModelChange)="onOperatorChange($event)" styleClass="p-column-filter-operator-dropdown"></p-select>
                    </div>
                    <div class="p-column-filter-constraints">
                        <div *ngFor="let fieldConstraint of fieldConstraints; let i = index" class="p-column-filter-constraint">
                            <p-select appendTo="body" appendTo="body" appendTo="body"  *ngIf="showMatchModes && matchModes" [options]="matchModes" [ngModel]="fieldConstraint.matchMode" (ngModelChange)="onMenuMatchModeChange($event, fieldConstraint)" styleClass="p-column-filter-matchmode-dropdown"></p-select>
                    ...
                    <div class="p-column-filter-buttonbar">
                        <button *ngIf="showClearButton" type="button" class="btn btn-danger m-0" (click)="clearFilter()">
                            <i class="pi pi-times-circle"></i>
                            {{clearButtonLabel}}
                        </button>
                        <button *ngIf="showApplyButton" type="button" class="btn btn-success m-0" (click)="applyFilter()">
    ...
    standalone:true,
    imports:[CommonModule,FormsModule,ButtonModule,SelectModule,ColumnFilterFormElement],    
})
export class ColumnFilter implements AfterContentInit {
    ...
    constructor(public el: ElementRef, public dt: Table, public renderer: Renderer2, public cdr: ChangeDetectorRef, public config: PrimeNG, public overlayService: OverlayService) { }
        ...
        if (!this.documentClickListener) {
            ...
            this.documentClickListener = this.renderer.listen(documentTarget, 'mousedown', event => {
                if (this.overlayVisible && !this.selfClick && this.isOutsideClicked(event)) {
                    // this.hide();
                    // added by mozahed beacuse changes not affect overlay menu
    ...
    bindDocumentResizeListener() {
        ...
        this.documentResizeListener = () => { //Toukir
...
@NgModule({
    imports: [CommonModule, PaginatorModule, InputTextModule, SelectModule, ScrollingModule, FormsModule, ButtonModule, SelectButtonModule, CalendarModule, InputNumberModule, CheckboxModule
        ,Table, SharedModule, SortableColumn, FrozenColumn, RowGroupHeader, SelectableRow, RowToggler, ContextMenuRow, ResizableColumn, ReorderableColumn, EditableColumn, CellEditor, SortIcon,
    ...
    exports: [CommonModule, PaginatorModule, InputTextModule, SelectModule, ScrollingModule, FormsModule, ButtonModule, SelectButtonModule, CalendarModule, InputNumberModule, CheckboxModule,
        Table, SharedModule, SortableColumn, FrozenColumn, RowGroupHeader, SelectableRow, RowToggler, ContextMenuRow, ResizableColumn, ReorderableColumn, EditableColumn, CellEditor, SortIcon,
        TableRadioButton, TableCheckbox, TableHeaderCheckbox, ReorderableRowHandle, ReorderableRow, SelectableRowDblClick, EditableRow, InitEditableRow, SaveEditableRow, CancelEditableRow, ScrollingModule, ColumnFilter],
